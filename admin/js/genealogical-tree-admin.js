	...
	$(window)
		.on('load', function () {
			...
			$('.generate_default_tree')
				...
				$('#gt-family-groupchecklist input').each(function(){
					var val = $(this).attr('value');
					if(isInArray(val, gtObj.gt_family_group)) {
						$(this).parents('li').show()
					}
				})

				$('#gt-family-groupchecklist-pop input').each(function(){
					var val = $(this).attr('value');
					if(isInArray(val, gtObj.gt_family_group)) {
						$(this).parents('li').show()
					}
			...
			var button = $(this),
				...
				/* loop through all the images */
					...
					if( !in_array( attachments[i].id, hiddenfieldvalue ) ) {
						...
						$('ul.misha_gallery_mtb').append('<li data-id="' + attachments[i].id + '">\
